...
//            System.out.println(c);
    ...
    public static void find_det(Key k){
        ...
        System.out.println("det: " + k.getDet());
    }

    public static int[][] matrix_dop(Key k){
...
//            k.put_key_matr(2, 2, (a00*a11 - a01*a10));
    ...
    public static void trans_matr(Key k){
        int rev_det = reverse_element(k);
        System.out.println("Обратный элемент равен: " + rev_det);
        int a[][] = matrix_dop(k);
        int razr = k.getRazryadnost();
        int[][] transp_matr = new int[razr][razr];
        for(int i = 0; i<3; i++){
            for(int j = 0; j< 3; j++){
                int n = a[i][j] % 37;
                n *= rev_det;
        ...
        System.out.println("После деления с остатком, умножения на обратный элемент, повторного деления с остатком и транспонирования: ");
        ...
        System.out.println("Получена обратная по модуля матрица ключа");
    }
...
//        calculate_output_matr(message1, key1);
        shift_to_n_blocks_to_matr("иб ыхзц?р", key1);
        find_det(key1);
        trans_matr(key1);
    }
}
